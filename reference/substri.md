...
`substri` splits each element in 'x' into substrings and returns the
desired `substring`. 'sep' defines the delimiters separating the strings
into substrings, such that the input data between the matches become the
fields themselves. 'pos' defines the substring index(es) to be returned.
...
[`str_split`](https://stringr.tidyverse.org/reference/str_split.html),[`str_replace_all`](https://stringr.tidyverse.org/reference/str_replace.html)
