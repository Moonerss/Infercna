.cnaCor <- function(cna, cor.method = "pearson", cell.quantile = NULL, gene.quantile.for.cells = NULL, refCells = NULL, na.replace = NULL) {
  ...
  if (!is_null(na.replace)) {
    cellcors[is_na(cellcors)] <- na.replace
  }
  cellcors
}
...
#' @param cell.quantile calculate CNA measures including only top / "hotspot" genes according to their squared CNA values across all cells. Value between 0 and 1 denoting the quantile of genes to include. Default: NULL
#' @param gene.quantile.for.cells calculate CNA measures including only top / "hotspot" genes according to their squared CNA values across all genes Value between 0 and 1 denoting the quantile of genes to include, This is matched with `cell.quantile`. Default: NULL
...
#' @param cell.quantile.for.genes calculate CNA measures including only top / "hotspot" genes according to their squared CNA values across all genes Value between 0 and 1 denoting the quantile of genes to include, This is matched with `gene.quantile`. Default: NULL
...
#'
#' @importFrom rlang is_null is_true is_list
...
cnaCor <- function(cna, cor.method = "pearson",
  ...
  cors <- Unlist(cors, nested.names = TRUE)
}
