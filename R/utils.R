...
#'
#' @title Apply a Function to All Pairs of Elements in \code{x} or Between \code{x} and \code{y}
#' @description Apply a Function to All Pairs of Elements in \code{x} or Between \code{x} and \code{y}
#'
...
#' @param FUN the function to be applied to every pair of elements in \code{x}, or to every pair of elements between \code{x} and \code{y}. The function should take two arguments and return a numeric value. Default: intersect
...
#' @export
#'
comply <- function(x, y = NULL, FUN = intersect, ..., simplify = TRUE) {
  ...
  rows <- names(y)
  cols <- names(x)
  m <- outer(seq_along(x), seq_along(y),
