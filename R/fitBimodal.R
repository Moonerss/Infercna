...
fitBimodal <- function(x,
  ...
  if (length(x) < size * 2) {
    cli::cli_abort('Number of observations is too small for 2 modes >= {.val {size}}')
  }

  obj <- suppressWarnings(nor1mix::norMixEM(x, m = 2, maxit = maxit))

  if (is_false(attr(obj, 'converged'))) {
    if (verbose) cli::cli_alert_info('No bimodal distribution found.')
    return(FALSE)
  }
  ...
  if (any(colSums(passed) < size)) {
    if (verbose) cli::cli_alert_info('At least one mode contains < ', size, ' obs.')
    return(FALSE)
  }

  observed <- (sum(colSums(passed))) / length(x)
  if (observed < coverage) {
    if (verbose) cli::cli_alert_info('Less than {.val {coverage * 100}}% of obs. could be assigned to a mode.')
    return(FALSE)
  }

  if (verbose) cli::cli_alert_info('Success!')
