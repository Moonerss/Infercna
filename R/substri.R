...
#' @description \code{substri} splits each element in 'x' into substrings and returns the desired \code{substring}. 'sep' defines the delimiters separating the strings into substrings, such that the input data between the matches become the fields themselves. 'pos' defines the substring index(es) to be returned.
#'
...
substri <- function(x,
  ...
  if (na.rm) {
    # remove NA positions
    strings <- sapply(strings, function(s) s[!is_na(s)], simplify = F)
  }
